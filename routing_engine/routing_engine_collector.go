...
func init() {
	...
	cpuSystem = prometheus.NewDesc(prefix+"cpu_system", "Percentage of CPU time being used by kernel processes", l, nil)
	cpuInterrupt = prometheus.NewDesc(prefix+"cpu_interrupt", "Percentage of CPU time being used by interrupts", l, nil)
	cpuIdle = prometheus.NewDesc(prefix+"cpu_idle", "Percentage of CPU time that is idle", l, nil)
